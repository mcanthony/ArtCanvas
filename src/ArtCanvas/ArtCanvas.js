(function(global) {
    ...
    function ArtCanvas(container, canvas, width, height, callbacks) {
        ...
        this.textStyle = new Mocks.ArtCanvas.TextStyle(new Mocks.ArtCanvas.Font('Arial', 'normal', '16px'), new Mocks.ArtCanvas.Color(0, 0, 0, 1.0).toString());
        ...
        var _figure = function(activeCanvas, activeContext, x, y, event) {
            ...
            switch (self.figure) {
                case Mocks.ArtCanvas.Figure.RECTANGLE :
                    var left   = point.getX();
                    var top    = point.getY();
                    var offset = Mocks.ArtCanvas.Point.getOffsetPoint(point, new Mocks.ArtCanvas.Point(x, y));
                    ...
                    if (/mouseup|touchend/i.test(event)) {
                        ...
                        activeCanvas.pushPath(new Mocks.ArtCanvas.Rectangle(left, top, offset.getX(), offset.getY()));
                    }
                ...
                case Mocks.ArtCanvas.Figure.CIRCLE :
                    ...
                    var radius  = Mocks.ArtCanvas.Point.getDistance(point, new Mocks.ArtCanvas.Point(x, y));
                    ...
                    if (/mouseup|touchend/i.test(event)) {
                        activeCanvas.popPath();
                        activeCanvas.pushPath(new Mocks.ArtCanvas.Circle(centerX, centerY, radius, 0, (2 * Math.PI), false));
                    }
                ...
                case Mocks.ArtCanvas.Figure.LINE :
                    ...
                    if (/mouseup|touchend/i.test(event)) {
                        activeCanvas.popPath();
                        activeCanvas.pushPath(new Mocks.ArtCanvas.Line(point, new Mocks.ArtCanvas.Point(x, y)));
                    }
        ...
        this.container.addEventListener(Mocks.ArtCanvas.MouseEvents.START, function(event) {
            if (self.mode === Mocks.ArtCanvas.Mode.TOOL) {
                return;
            }
            ...
            if (self.mode === Mocks.ArtCanvas.Mode.ERASER) {
            ...
            switch (self.mode) {
                ...
                case Mocks.ArtCanvas.Mode.TEXT :
                    activeCanvas.createTextbox(new Mocks.ArtCanvas.Point(x, y));
                    return;
            ...
            activeCanvas.pushPath([new Mocks.ArtCanvas.Point(x, y)]);
        ...
        this.container.addEventListener(Mocks.ArtCanvas.MouseEvents.MOVE, function(event) {
            ...
            switch (self.mode) {
                ...
                case Mocks.ArtCanvas.Mode.ERASER :
                    ...
                    switch (self.mode) {
                        case Mocks.ArtCanvas.Mode.HAND :
                            points.push(new Mocks.ArtCanvas.Point(x, y));
                            break;
                        case Mocks.ArtCanvas.Mode.ERASER :
                            points.push(new Mocks.ArtCanvas.Eraser(new Mocks.ArtCanvas.Point(x, y)));
                            break;
                        default :
                            break;
                    }
    ...
    ArtCanvas.prototype.translate = function(translateX, translateY) {
        var canvas = this.layers[this.activeLayer];
        canvas.transform(Mocks.ArtCanvas.Transform.TRANSLATE, [translateX, translateY]);
    ...
    ArtCanvas.prototype.scale = function(scaleX, scaleY) {
        var canvas = this.layers[this.activeLayer];
        canvas.transform(Mocks.ArtCanvas.Transform.SCALE, [scaleX, scaleY]);
    ...
    ArtCanvas.prototype.rotate = function(degree) {
        var canvas = this.layers[this.activeLayer];
        canvas.transform(Mocks.ArtCanvas.Transform.ROTATE, [degree]);
